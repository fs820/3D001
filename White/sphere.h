...
////全球
//#define VT_MAX_SP ((SPHERE_HNUM + 1) * (SPHERE_VNUM-1)+2)//横の頂点数*縦のライン数(2極を抜かす)+2極 (全球)
//#define INDEX_SP_NUM (((SPHERE_HNUM+1)*2)*(SPHERE_VNUM-2)+(SPHERE_VNUM-3)*2+(SPHERE_HNUM+2)*2)//横の頂点数*2列**縦のライン数(2極を抜かす)+縮退ポリゴン+2極
//#define POLYGON_SP_NUM (SPHERE_HNUM*(SPHERE_VNUM-2)*2+(SPHERE_VNUM-3)*4+SPHERE_HNUM*2)//横の分割*2*縮退ポリゴン+2極
